...
public class LRAssistMode extends AutoModeBase{
	Superstructure s;
	Intake intake;

	private static List<Trajectory<TimedState<Pose2dWithCurvature>>> paths = Arrays.asList(trajectories.frontLeftSwitch,
		trajectories.frontLeftSwitchToOuterCube, trajectories.outerCubeToFrontLeftSwitch, trajectories.frontLeftSwitchToMiddleCube,
		trajectories.alternateMiddleCubeToRightScale);
	...
	protected void routine() throws AutoModeEndedException {
        ...
        runAction(new SetTrajectoryAction(trajectories.alternateMiddleCubeToRightScale, -90.0, 0.5));
        runAction(new WaitAction(0.5));
        s.request(s.wristIntakeConfig(Constants.kWristPrimaryStowAngle, IntakeState.CLAMPING));
        runAction(new WaitToPassXCoordinateAction(Constants.kRightSwitchFarCorner.x() - 1.0));
